...
#else
// ATmega32U4 can be Teensy 2.0, Adafruit breakout board or Pro Micro; default
// to Teensy 2.0
#if !defined(LAYOUT_PRO_MICRO)
#define LAYOUT_PRO_MICRO 0
#endif
...
#define LLED_ON(b) \
  (DDRF |= LLED_BITMASKD, PORTF = (PORTF & ~LLED_BITMASKP) | ((b) << 5))
#define LLED_OFF(b) \
  (DDRF = (DDRF & (~LLED_BITMASKD)) | ((b) << 5), PORTF |= LLED_BITMASKP)

#define AUXK_BITMASKD \
  ((1 << DDB4) | (1 << DDB3) | (1 << DDB2) | (1 << DDB1) | (1 << DDB0))
#define AUXK_BITMASKP                                              \
  ((1 << PORTB4) | (1 << PORTB3) | (1 << PORTB2) | (1 << PORTB1) | \
   (1 << PORTB0))
...
#define AUXK_BITMASKD \
  ((1 << DDB5) | (1 << DDB4) | (1 << DDB3) | (1 << DDB2) | (1 << DDB1))
#define AUXK_BITMASKP                                              \
  ((1 << PORTB5) | (1 << PORTB4) | (1 << PORTB3) | (1 << PORTB2) | \
   (1 << PORTB1))
#define AUXK_CONFIG (DDRB &= ~AUXK_BITMASKD, PORTB |= AUXK_BITMASKP)
...
#define LLED_ON(b) \
  (DDRD |= LLED_BITMASKD, PORTD = (PORTD & ~LLED_BITMASKP) | ((b) << 3))
#define LLED_OFF(b) \
  (DDRD = (DDRD & (~LLED_BITMASKD)) | ((b) << 3), PORTD |= LLED_BITMASKP)
