...
static const uint8_t PROGMEM endpoint_config_table[] = {
    ...
    1, EP_TYPE_INTERRUPT_IN, EP_SIZE(RAWHID_TX_SIZE) | RAWHID_TX_BUFFER, 1,
    EP_TYPE_INTERRUPT_OUT, EP_SIZE(RAWHID_RX_SIZE) | RAWHID_RX_BUFFER
#endif
};
...
static const uint8_t PROGMEM debug_hid_report_desc[] = {
    0x06,
    LSB(DEBUGHID_USAGE_PAGE),
    MSB(DEBUGHID_USAGE_PAGE),  // Usage Page 0xFF31 (vendor defined)
    0x09,
    DEBUGHID_USAGE,  // Usage 0x74
    0xA1,
    0x53,  // Collection 0x53
    0x75,
...
// A_0511
static const struct usb_string_descriptor_struct PROGMEM string0 = {4,
                                                                    3,
                                                                    {0x0409}};
// A_0515
...
#define CONFIG1_DESC_SIZE                                               \
  (CFG_DESC_SZ + BOOT_HID_DESC_SZ + KBD_HID_DESC_SZ + DBG_HID_DESC_SZ + \
   RAW_HID_DESC_SZ)
...
#define DEBUG_HID_DESC_OFFSET \
  (CFG_DESC_SZ + BOOT_HID_DESC_SZ + KBD_HID_DESC_SZ + IF_DESC_SZ)
#define RAWHID_HID_DESC_OFFSET                                          \
  (CFG_DESC_SZ + BOOT_HID_DESC_SZ + KBD_HID_DESC_SZ + DBG_HID_DESC_SZ + \
   IF_DESC_SZ)
...
#endif
#if INC_IF_KBD
    {0x2200, IF_Kbd, keyboard_hid_report_desc,
...
struct                       // still slightly shady area
{
  ...
  uint8_t
      data3[3];  // 0x0168 HIDX KEY_MEDIA_NEXT_TRACK..KEY_MEDIA_WWW_FAVORITES
...
int8_t usb_debug_putchar(uint8_t c)
{
  ...
  if (!usb_configuration || usb_suspended || usb_resuming || !keyboard_protocol)
    return -1;
