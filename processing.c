...
uint8_t* fn_keyset =
    0;  // 0x0175/6 bitmask of FN modifiers for each pressed key
...
uint8_t* macro_data = 0;   // 0x0178/9 5-byte tuples
                           ...
                           //                  1st 2 bytes there are counts
                           //                  (0..63) of 2-byte tuples
...
uint8_t* modif_keyset =
    0;  // 0x029a/b bitmask of modifiers for each translated key
...
uint8_t pressed_hidxs[32] = {
    0};  // 0x02e3..0x0302 bitmask of currently pressed HIDXs
...
uint8_t translate_hidx(uint8_t keyid, uint8_t make)
{
  ...
  if (max_used_layer)  // if layers used
  {
    ...
    if (make)
      fn_keyset[keyid] =
          fn_curset;  // remember current FN set with unmodified key
    // A_23c2
    // R2829 = keyid;
    // A_23c6
...
uint8_t alloc_layers(uint8_t layers, uint8_t max_used)
{
  ...
  layerdata =
      (uint8_t*)memalloc(2 * layers + (256 + sizeof(uint8_t*)) * max_used);
  if (!layerdata)
    return 0;
  // A_25f8
  ...
  | uninitialized |   pointers   |   area 1         | ...  | area (max_used -
  1)| | ............. | pp pp pp pp  | 0000000000000000 |      |
  000000000000000000 | ^ |             ^                         ^ |
  +-------------+------------------...----+ layer_map -------+

  The start of layerdata is filled with a set of (FN key HIDX, layer#) tuples
  in setup_layer(). */
...
uint16_t find_macro(uint8_t hidx, uint8_t modif_set)
{
  ...
  for (i = 0, pmacro = macro_data; i < macro_cnt; i++, pmacro += 5) {
    ...
    if (*pmacro == hidx) {
      ...
      modif_set |=
          cmset;  // Not sure whether this is good ... but definitely so
      if (!((b1 ^ modif_set) & b2))
...
void rawhid_comm(void)
{
  ...
  switch (cur_commstate) {
            ...
            static const uint8_t PROGMEM info_templ[] =  // A_060d
                {
                    0x03,
                    PROTOCOL_VER,
                    PROTOCOL_SUBVER,  // protocol version
                    0x01,
                    CODE_VER,
                    CODE_SUBVER,  // code version (v1.12)
                    0x02,
                    SETTINGS_VER,
                    SETTINGS_SUBVER,  // max settings version (v1.01)
                    0x04,
                    0x00,
                    0x00,  // filled with current settings version (v1.01)
                    0x05,
                    LSB(RAMEND + 1),
                    MSB(RAMEND + 1),  // SRAM size
                    0x07,
                    0x00,
                    0x00,  // filled with # free memory bytes
                    0x06,
                    LSB(E2END + 1),
                    MSB(E2END + 1),  // EEPROM size
                    0x08,
                    0x00,
                    0x00,  // filled with free EEPROM memory
                    0x00   // EOB
                };
            const uint8_t* s;
            uint8_t* t;
            for (s = info_templ, t = comm.rcvbuf;
                 s < info_templ + sizeof(info_templ); s++, t++)
              *t = pgm_read_byte(s);
            // A_3028
            eecnt = eeprom_read_word((const uint16_t*)2);
            if (eecnt) {
    ...
    case csCEEP:  // compare EEPROM to buffer
      ...
      // R13 = xfer_buf_len;
      {
        ...
        uint16_t eeaddr;
        // A_31f8
        for (eeaddr = xfer_eeaddr; eeaddr - xfer_eeaddr < xfer_buf_len;
             eeaddr++) {
    ...
    case csREEP:  // this one sends a block of EEPROM data
      ...
      if (comm.rcvbuf[0] == 3) {
        // A_3294
        uint8_t icnt = (xfer_len > sizeof(comm.rcvbuf)) ? sizeof(comm.rcvbuf)
                                                        : (uint8_t)xfer_len;
        if (icnt) {
          // A_32ac
        ...
        // A_32ee
        rawhid_commstate = csREEP;
      } else if (comm.rcvbuf[0] == 4 || comm.rcvbuf[0] == 1)
        rawhid_commstate = csIdle;
      else
        // A_32ee
        rawhid_commstate = csREEP;
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
  tcnow = timer0_counter;
  if (rawhid_commstate == csIdle || rawhid_commstate != cur_commstate)
    // A_3358
    tcnow_start = tcnow;
  // A_336a
...
{
  return keyboard_leds;  // a bit weird, since the data are definitely in USB
                         // code
}
...
{
  return keyboard_protocol;  // a bit weird, since the data are definitely in
                             // USB code
}
...
void incoming_keybreak(uint8_t keyid)
{
  ...
  if (keyid && !find_exec_macro(keyid, 0))
...
void incoming_keymake(uint8_t keyid)
{
  ...
  if (keyid && !find_exec_macro(keyid, 1))
...
void macro_tick(void)
{
  ...
  if (usb_suspended || usb_resuming) {
    ...
    // A_3570
    if (!usb_resuming && key_queue_widx != key_queue_ridx) {
      // A_357a
